...
class Controller(object):
    ...
    def set_registry(self, rule_group):
        ...
        if rule_group in self._registry.get(rule_group.app_label):
            raise AlreadyRegistered('The rule {0} is already registered for module {1}'.format(
                rule_group.__name__, rule_group.app_label))
    ...
    def register(self, rule_group):
        ...
        groups were declared in; which is the same module as the visit model (see update)."""
